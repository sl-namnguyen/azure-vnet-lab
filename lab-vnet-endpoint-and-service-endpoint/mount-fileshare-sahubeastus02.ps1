...
$connectTestResult = Test-NetConnection -ComputerName sahubeastus02.file.core.windows.net -Port 445
if ($connectTestResult.TcpTestSucceeded) {
    # Save the password so the drive will persist on reboot
    cmd.exe /C "cmdkey /add:`"$storageAccountName.file.core.windows.net`" /user:`"localhost\$storageAccountName`" /pass:`"$storageAccountKey`"" # thay key vào đây
    # Mount the drive
    New-PSDrive -Name X -PSProvider FileSystem -Root "\\$storageAccountName.file.core.windows.net\test" -Persist
} else {
